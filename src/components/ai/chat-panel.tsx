...
export function ChatPanel({ isOpen, onClose, variant = "panel" }: ChatPanelProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center px-6">
        {onClose && (
          <button
            onClick={onClose}
            className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground hover:text-white hover:bg-[#1a1a1a] transition-colors"
          >
            <X className="h-4 w-4" />
          </button>
        )}
        ...
        <p className="text-muted-foreground max-w-md mb-6">
          AI features require an OpenAI API key. Add{" "}
          <code className="bg-[#1a1a1a] px-1.5 py-0.5 rounded text-sm">OPENAI_API_KEY</code> to
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {variant === "full" ? (
        ...
        <div className="flex items-center justify-between shrink-0 px-5 py-4 border-b border-white/[0.06]">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground hover:text-white hover:bg-[#1a1a1a] transition-colors"
            >
              <X className="h-4 w-4" />
            </button>
          )}
      ...
      <div className={cn(
        ...
        variant === "full" ? "px-1 pb-4" : "px-5 py-4"
      )}>
        {messages.length === 0 && (
      ...
      <div className={cn(
        "shrink-0 py-4",
        variant === "full" ? "px-1" : "px-5 border-t border-white/[0.06]"
      )}>
        <div className="relative">
          <Input
            ...
            disabled={loading}
            className="pr-12 py-5 text-sm bg-[#111111] border-white/[0.06] rounded-2xl focus-visible:ring-violet-500/20 placeholder:text-muted-foreground/50"
          />
          <Button
            size="icon"
